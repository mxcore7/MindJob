...
const handleSearch = () => {
  ...
  searchTimeout = setTimeout(() => {
    jobsStore.fetchJobs({ search: search.value, location: location.value, page: 1 })
  }, 500)
}

const handlePageChange = (page) => {
  if (page < 1 || page > jobsStore.totalPages) return
  jobsStore.fetchJobs({ search: search.value, location: location.value, page })
}
</script>
...
<template>
  <div class="px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div v-else-if="!jobsStore.jobs.length" class="glass rounded-2xl flex flex-col items-center justify-center py-20 text-center">
      ...
      <button @click="search = ''; location = ''; handleSearch()" class="mt-4 btn-gradient px-4 py-2 rounded-lg text-xs cursor-pointer">Clear Filters</button>
    </div>
